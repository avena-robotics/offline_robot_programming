...
class RobotPost(object):
    ...
    def SaveTrajToCsv(self, file_path: str, trajectory: list):
        csv_data = ''
        time_from_start = 0.0
        for jtp in trajectory:
            angles = jtp[:self.nAxes]
            vel = jtp[14:14+self.nAxes]
            acc = jtp[20:20+self.nAxes]
            time_step = jtp[10]
            time_from_start += int(time_step * RobotPost.SEC_TO_MS)
            angles_str = list(map(lambda x : str(int(x * RobotPost.DEG_TO_RAD / RobotPost.JOINT_POSMAX * RobotPost.MAX_INT16)), angles))
            vel_str = list(map(lambda x : str(int(x * RobotPost.DEG_TO_RAD / RobotPost.JOINT_VELMAX * RobotPost.MAX_INT16)), vel))
            acc_str = list(map(lambda x : str(int(x * RobotPost.DEG_TO_RAD / RobotPost.JOINT_ACCMAX * RobotPost.MAX_INT16)), acc))
            # Append data to line
            csv_data += str(time_from_start) + ',' + ','.join(angles_str) + ',' + ','.join(vel_str) + ',' + ','.join(acc_str) + '\n'
    ...
    def ProgSave(self, folder, progname, ask_user=False, show_result=False):
        ...
        **Tip**:
        ...
        if ask_user or not DirExists(folder):
            ...
            import os
            filesave = os.path.join(folder, full_progname)
        ...
        ###############################################################        
        program = self.RDK.Item(progname)
        ...
        #     os.remove(filesave)
        #     return
        # self.RDK.ShowMessage(f'Successfully saved trajectory to: {filesave}')
        ...
        traj = traj.list2()
        if len(traj) < 2:
            self.RDK.ShowMessage('Cannot replay robot path because number of trajectory points is too low.')
            return

        self.SaveTrajToCsv(filesave, traj)
        
        # robot = self.RDK.Item('avena_arm')
        # for jtp in traj:
        #     joint_angles, time_step = jtp[:6], jtp[10]
...
def test_post():
    ...
    r.MoveJ(p(200, 200, 500, 180, 0, 180), [-46.18419, -6.77518, -20.54925, 71.38674, 49.58727, -302.54752])
    r.MoveL(p(200, 250, 348.734575, 180, 0, -150), [-41.62707, -8.89064, -30.01809, 60.62329, 49.66749, -258.98418])
    r.MoveL(p(200, 200, 262.132034, 180, 0, -150), [-43.73892, -3.91728, -35.77935, 58.57566, 54.11615, -253.81122])
    r.RunMessage("Setting air valve 1 on")
    r.RunCode("TCP_On", True)
    r.Pause(1000)
    r.MoveL(p(200, 250, 348.734575, 180, 0, -150), [-41.62707, -8.89064, -30.01809, 60.62329, 49.66749, -258.98418])
    r.MoveL(p(250, 300, 278.023897, 180, 0, -150), [-37.52588, -6.32628, -34.59693, 53.52525, 49.24426, -251.44677])
    r.MoveL(p(250, 250, 191.421356, 180, 0, -150), [-39.75778, -1.04537, -40.37883, 52.09118, 54.15317, -246.94403])
    r.RunMessage("Setting air valve off")
    r.RunCode("TCP_Off", True)
    r.Pause(1000)
    r.MoveL(p(250, 300, 278.023897, 180, 0, -150), [-37.52588, -6.32628, -34.59693, 53.52525, 49.24426, -251.44677])
    r.MoveL(p(250, 200, 278.023897, 180, 0, -150), [-41.85389, -1.95619, -34.89154, 57.43912, 52.34162, -253.73403])
    r.MoveL(p(250, 150, 191.421356, 180, 0, -150), [-43.82111, 3.29703, -40.29493, 56.02402, 56.61169, -249.23532])
